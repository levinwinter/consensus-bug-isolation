...
Phase 1 - Cache Generation:
  ...
  - Evaluates ~360,000 predicates on message pairs per node
...
from final import generate_predicate_cache, load_predicate_cache
...
_SET_LOW = frozenset([0, 1, 2, 3, 4])   # Left partition
_SET_HIGH = frozenset([2, 3, 4, 5, 6])  # Right partition

# Cache file paths for aggregation results
AGGREGATION_CACHE_FILE = '1200filename.pickle'
REPORTS_CACHE_FILE = '1200filename2.pickle'
...
if __name__ == '__main__':
    ...
    if REGENERATE_CACHE:
        ...
        # Filter to only process runs listed in runs.py
